...
class PromptService:
    def __init__(self):
        ...
        self.prompt_collection: List[Prompt] = [check_barcode_is_PR, DO3]
...
check_barcode_is_PR = Prompt(
    id=1,
    data="""Прочитай изображение штрих-кода. Если на штрих-коде или рядом с ним есть надпись "ПОЧТА РОССИИ" - верни True. Иначе - False.
...
DO3 = Prompt(
    ...
    - НомерДокумента (определяется по приоритету):
    
        1. Госорганы/Входящие: Если отправитель — госорган (суд, прокуратура, ФССП) или на конверте есть «Вх. №» — возьми номер с конверта.

        2. Исходящий: Если отправитель — коммерческая организация или физлицо — ищи в шапке документа «Исх. №».

        3. Альтернатива: Если «Исх. №» нет, возьми любой номер из шапки (например, «№ 123-ОП»).


    
    - Даты: Ищи "Дата". Дата должна быть в формате ГГГГММДД.
